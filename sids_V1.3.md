...
- 数据范围：选取 Kaggle 社区基于 CSE-CIC-IDS2018 二次整理的抽样混合流量数据集（涵盖正常流量、DoS/DDoS 与暴力破解核心场景）；
...
选取开源社区（如 Kaggle）针对 CSE-CIC-IDS2018 重新清洗并按比例重采样的综合数据集文件
...
> **注意**：Docker Compose V2 已废弃 `version` 属性，故以下配置不再声明版本。
...
> **⚠️ 重要说明**：官方的 `apache/spark:3.5.0-python3` 镜像将 `spark-submit` 存放在 `/opt/spark/bin/` 目录下。当你使用 `docker compose exec` 从外部直接向容器发送指令时，容器并不会像正常登录终端那样加载环境变量（如将 `/opt/sppark/bin` 加入 `$PATH`），因此必须使用**绝对路径**。
...
- **依赖解耦**：由于 `utils.py` 被 Spark 容器（无 pandas）和 Web 容器（需 pandas）共享，故将 pandas/numpy 导入移至函数内部实现"懒加载"
...
- Spark内存配置：driver.memory=6G，executor.memory=6G，适配24GB单机环境；容器内存限额已提升至12G，确保单个组件声明内存不超过容器水位。
- 核心字段数量：10个核心特征，兼顾模型性能与计算效率，经过特征选择后的精简集；
- 存储配置：Parquet格式采用snappy压缩（默认），保留多分片输出以降低写入内存压力，DuckDB 使用通配符读取多个 Parquet 分片。

### 5.5.3 问题排查记录（实践修正）

> 本节记录实际开发中遇到的问题及解决方案，供后续参考。

#### 问题1：spark-submit 找不到
...
**原因**：官方的 `apache/spark:3.5.0-python3` 镜像将 `spark-submit` 放在 `/opt/spark/bin/` 目录下，但 `docker compose exec` 不加载环境变量 `$PATH`。

**解决方案**：修改 `run_etl.sh`，使用绝对路径：
```bash
docker compose exec spark /opt/spark/bin/spark-submit \
    --master local[*] \
    /app/spark_jobs/etl_clean.py
...
**原因**：`utils.py` 被 Spark 容器引用时执行了顶层 `import pandas`，但 Spark Python 镜像不含 pandas。

**解决方案**：将 pandas/numpy 导入移至函数内部（懒加载），仅在调用时导入。

#### 问题3：MIXED_TYPE_REPLACEMENT 错误
...
**原因**：Spark 3.x 对类型检查极其严格，使用 `na.replace()` 或 `df.replace()` 进行跨类型替换（如 String→Integer）会报错。

**解决方案**：
1. **load_data 函数**：放弃 `na.replace()`，直接使用 `cast("double")` —— Spark 会自动将 "Infinity"/"NaN" 转为 null
2. **unify_label 函数**：使用**同构替换**（String→String），映射值为字符串 `"0"/"1"/"2"`，最后统一 `cast("integer")`
...
**原因**：文档中 `save_processed_data` 函数使用了 2 空格缩进，与其他函数 4 空格不一致。
