...
- 技术层：基于Spark实现并行数据处理，基于XGBoost构建攻击检测模型，攻击识别准确率目标为85%（注：数据不平衡可能影响准确率，后续可通过加权或重采样优化，此目标为工程可达指标）；
...
- 预计算统计：离线计算攻击类型分布、Top10源IP等核心指标，保存为JSON文件供前端快速调用；
- 数据存储：清洗后的数据集以Parquet格式存储，提升数据读取效率。

### 2.1.2 模型训练与预测功能

- 特征筛选：选取10个核心网络流量特征（如Flow Duration、Total Fwd Packets等）用于模型训练；
...
- 大屏展示：通过ECharts实现攻击类型分布饼图、流量趋势折线图、Top10源IP柱状图等可视化图表；
...
- 可视化大屏页：核心展示界面，呈现攻击类型分布、Top10源IP等统计图表；
...
- 字段筛选：保留10个核心字段，包括Flow Duration、Total Fwd Packets、Total Backward Packets、Flow Bytes/s、Flow Packets/s、Total Length of Fwd Packets、Total Length of Bwd Packets、Fwd Packet Length Max、Fwd Packet Length Min、Fwd Packet Length Mean等（详见4.0节CORE_FEATURES配置）；
...
1. 数据读取：Spark读取原始CSV文件，通过显式定义的StructType结构加载数据，禁用inferSchema以优化I/O性能并确保Driver内存安全；
2. 数据清洗：将Infinity、NaN替换为null，过滤核心字段中的负数；
3. 缺失值处理：对核心特征字段执行fillna(0)零值填充；
4. 标签统一：基于映射表替换标签，删除未匹配标签的数据；
5. 预计算统计：计算攻击类型分布（各标签数量占比）、Top10源IP（按流量包数量排序），保存为dashboard_stats.json；
...
12. 模型评估：在测试集上验证模型性能，记录准确率、精确率、召回率等指标（目标准确率≥85%，数据不平衡可能影响实际表现，此为目标工程可达指标）；
...
16. 可视化展示：渲染攻击类型分布、Top10源IP等可视化图表，系统处于可观测状态；
  ...
  <div class="dashboard-container">
    ...
    <el-row :gutter="20" class="chart-row">
      ...
      <el-col :span="12">
        <el-card shadow="hover">
          <template #header>
            <div class="card-header">Top10源IP流量包数量</div>
          </template>
          <div class="chart-container" ref="top10IpChartRef"></div>
        </el-card>
      </el-col>
    </el-row>
  </div>
...
const loadStatsData = async () => {
  try {
    ...
    // 处理Top10 IP数据：对过长的 IP/标签进行截断以免 X 轴拥挤，并通过 tooltip 显示完整字符串
    const top10Ip = res.top10_source_ip;
    const ipKeys = Object.keys(top10Ip);
    const ipValues = Object.values(top10Ip);
    // 截断展示标签（超过15字符截取并添加省略号）
    const displayLabels = ipKeys.map(k => (k && k.length > 15) ? (k.slice(0, 12) + '...') : k);
    top10IpOption.value.xAxis.data = displayLabels;
    top10IpOption.value.series[0].data = ipValues;
    // 在 tooltip 中显示完整 IP/标签（用户将看到原始字符串）
    top10IpOption.value.tooltip = {
      trigger: 'axis',
      axisPointer: { type: 'shadow' },
      formatter: function (params) {
        // params[0] 对应单个柱子
        const idx = params[0].dataIndex;
        const fullLabel = ipKeys[idx];
        return fullLabel + '<br/>' + params[0].seriesName + ': ' + params[0].data;
      }
    };
    ...
    if (top10IpChart.value) {
      top10IpChart.value.setOption(top10IpOption.value);
    }
  } catch (error) {
...
const top10IpChartRef = ref(null);

onMounted(() => {
  attackDistChart.value = echarts.init(attackDistChartRef.value);
  top10IpChart.value = echarts.init(top10IpChartRef.value);
  loadStatsData();
});
</script>
```
...
| M3: 模型 | 编写`train_model.py` | `xgb_model.json`, `feature_importance.json`   | 准确率目标 > 85%（工程可达指标），特征 JSON 生成 |
